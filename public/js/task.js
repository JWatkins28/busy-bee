    ...
    if (event.target.hasAttribute('data-id')) {
        if (event.target.hasAttribute('data-status')) {
            ...
            if (taskStatus == "false") {
                ...
                const response = await axios.put(`/api/tasks/${task_id}`, { completed: true });
                ...
                if (response.status == 200) {
                    ...
                    const response2 = await axios.put(`/api/users/${user_id}`, { honey, completed_tasks })
                ...
                const response = await axios.put(`/api/tasks/${task_id}`, { completed: false });
    ...
    if (event.target.hasAttribute('data-id')) {
        if (event.target.hasAttribute('data-status')) {
            ...
            if (taskStatus == "false") {
                ...
                const response = await axios.put(`/api/subtasks/${task_id}`, { completed: true });
                ...
                const response = await axios.put(`/api/subtasks/${task_id}`, { completed: false });
